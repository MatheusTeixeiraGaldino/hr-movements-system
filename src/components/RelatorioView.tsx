...
function buildMovementPDFHtml(m: Movement): string {
  ...
  const statusColor  = isCanceled ? '#dc2626' : (allDone ? '#166534' : '#92400e');
  const statusBg     = isCanceled ? '#fee2e2' : (allDone ? '#dcfce7' : '#fef3c7');
  const statusText   = isCanceled ? '✕ CANCELADO' : (allDone ? '✓ APROVADO — TODOS OS PARECERES EMITIDOS' : '⏳ PENDENTE — AGUARDANDO PARECERES');
  const statusBorder = isCanceled ? '#fca5a5' : (allDone ? '#86efac' : '#fde68a');

  const detailRows = Object.entries(m.details)
    .filter(([key]) => key !== 'observation' && key !== 'employeeName')
    .map(([key, value]) => {
      const label = DETAIL_LABELS[key] || key;
      const val   = typeof value === 'string' && value.match(/^\d{4}-\d{2}-\d{2}$/) ? formatDate(value) : String(value || '—');
  ...
  if (isCanceled) {
    ...
    <div class="cancellation-box">
      ...
      <table class="info-table">
        ...
        <tr><td class="detail-label">Data/Hora</td><td class="detail-value">${formatDateTime(m.cancelamento?.cancelado_em)}</td></tr>
  ...
  if (!isCanceled) {
    teamSections = m.selected_teams.map(teamId => {
      ...
      const histRows = history.length > 1
        ? history.slice(0, -1).map((h, i) => `<tr><td style="color:#6b7280;font-size:9px;padding:3px 6px">${i + 1}ª versão</td><td style="color:#6b7280;font-size:9px;padding:3px 6px">${h.user_name}</td><td style="color:#6b7280;font-size:9px;padding:3px 6px">${formatDateTime(h.timestamp)}</td><td style="color:#6b7280;font-size:9px;padding:3px 6px">${h.action === 'created' ? 'Emissão inicial' : 'Atualização'}</td></tr>`).join('') : '';
      const checklist = resp?.checklist || {};
      const checkItems = Object.entries(checklist);
      const checkHtml = checkItems.length > 0
        ? `<div class="checklist-wrap"><p class="checklist-title">Checklist</p>${checkItems.map(([item, checked]) => `<div class="check-item"><span class="check-icon" style="color:${checked ? '#16a34a' : '#dc2626'}">${checked ? '✓' : '✗'}</span><span style="color:${checked ? '#111' : '#6b7280'}">${item}</span></div>`).join('')}</div>` : '';

      return `
        <div class="team-card" style="border-color:${border};background:${bg}">
          <div class="team-header"><div><span class="team-name">${teamName}</span></div><span class="status-badge" style="color:${color};background:${done ? '#dcfce7' : '#fef3c7'};border:1px solid ${border}">${done ? '✓ Parecer Emitido' : '⏳ Pendente'}</span></div>
          ${done ? `
          <table class="info-table" style="margin-bottom:8px">
            <tr><td class="detail-label">Responsável</td><td class="detail-value">${pareceristaNome}${pareceristaMail ? ` <span style="color:#6b7280;font-size:9px">(${pareceristaMail})</span>` : ''}</td></tr>
            <tr><td class="detail-label">Data / Hora do Parecer</td><td class="detail-value" style="font-weight:700;color:#1e3a5f">${dataHoraParecer} <span style="font-size:9px;color:#6b7280">(${acaoLabel})</span></td></tr>
          </table>
          ${checkHtml}
          <div class="parecer-box"><p class="parecer-label">Parecer / Observações</p><p class="parecer-text">${resp?.comment || '—'}</p></div>
          ${histRows ? `<div style="margin-top:8px"><p style="font-size:9px;font-weight:700;color:#6b7280;margin-bottom:4px;text-transform:uppercase;letter-spacing:.5px">Histórico de alterações</p><table style="width:100%;border-collapse:collapse;font-size:9px"><thead><tr style="background:#f3f4f6"><th style="text-align:left;padding:3px 6px;color:#6b7280">Versão</th><th style="text-align:left;padding:3px 6px;color:#6b7280">Usuário</th><th style="text-align:left;padding:3px 6px;color:#6b7280">Data / Hora</th><th style="text-align:left;padding:3px 6px;color:#6b7280">Ação</th></tr></thead><tbody>${histRows}</tbody></table></div>` : ''}
  ...
  return `<!DOCTYPE html>
<html lang="pt-BR"><head><meta charset="UTF-8"/><title>Movimentação — ${m.employee_name}</title>
<style>
*{box-sizing:border-box;margin:0;padding:0}body{font-family:Arial,Helvetica,sans-serif;font-size:11px;color:#111;background:#fff;padding:28px 32px}
...
.doc-title{font-size:20px;font-weight:700;color:#1e3a5f}.doc-subtitle{font-size:11px;color:#555;margin-top:3px}
.doc-meta{text-align:right;font-size:10px;color:#555;line-height:1.6}.doc-meta strong{color:#111}
...
.section{margin-bottom:20px}.section-title{font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.8px;color:#1e3a5f;border-bottom:1.5px solid #bfdbfe;padding-bottom:4px;margin-bottom:10px}
.info-table{width:100%;border-collapse:collapse}.detail-label{width:36%;font-weight:600;color:#374151;padding:4px 8px 4px 0;vertical-align:top;white-space:nowrap}.detail-value{color:#111;padding:4px 0;vertical-align:top}
...
.cancellation-box{background:#fee2e2;border:1.5px solid #fca5a5;border-radius:8px;padding:12px 14px;margin-bottom:20px}.cancellation-title{font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:#7f1d1d;margin-bottom:8px}
.team-card{border:1.5px solid #e5e7eb;border-radius:8px;padding:12px 14px;margin-bottom:12px;page-break-inside:avoid}.team-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:8px}.team-name{font-size:12px;font-weight:700;color:#1e3a5f}.status-badge{font-size:10px;font-weight:700;padding:3px 10px;border-radius:20px}
.checklist-wrap{background:#fff;border:1px solid #e5e7eb;border-radius:6px;padding:8px 10px;margin-bottom:8px}.checklist-title{font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:#6b7280;margin-bottom:6px}.check-item{display:flex;align-items:flex-start;gap:6px;font-size:10px;margin-bottom:3px}.check-icon{font-size:11px;font-weight:700;flex-shrink:0;margin-top:1px}
.parecer-box{background:#fff;border:1px solid #e5e7eb;border-radius:6px;padding:8px 10px}.parecer-label{font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:#6b7280;margin-bottom:4px}.parecer-text{font-size:10px;color:#111;line-height:1.5;white-space:pre-wrap}
.summary-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:20px}.summary-card{background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:10px 12px;text-align:center}.summary-num{font-size:22px;font-weight:700;color:#1e3a5f}.summary-label{font-size:9px;color:#6b7280;text-transform:uppercase;letter-spacing:.5px;margin-top:2px}
...
@page{size:A4 portrait;margin:10mm 12mm}@media print{body{padding:0}.no-print{display:none}}
</style></head><body>
<div class="doc-header"><div><div class="doc-title">Ficha de Movimentação Trabalhista</div><div class="doc-subtitle">RH Movimentações — Registro Oficial</div></div><div class="doc-meta"><div>Nº do Registro: <strong>${m.id.substring(0, 8).toUpperCase()}</strong></div><div>Criado em: <strong>${criacao}</strong></div><div>Gerado em: <strong>${now}</strong></div></div></div>
<span class="status-geral" style="color:${statusColor};background:${statusBg};border:1.5px solid ${statusBorder}">${statusText}</span>
<div class="section"><div class="section-title">Identificação da Movimentação</div>
<table class="info-table">
...
<div class="doc-footer"><span>RH Movimentações — Documento gerado automaticamente em ${now}</span><span>ID: ${m.id}</span></div>
</body></html>`;
}
...
interface MultiSelectProps { label: string; selected: Set<string>; onToggle: (v: string) => void; onClear: () => void; options: MultiSelectOption[]; totalCount: number; }
...
function MultiSelect({ label, selected, onToggle, onClear, options, totalCount }: MultiSelectProps) {
  ...
  const displayLabel = hasSelection ? options.filter(o => selected.has(o.value)).map(o => o.label).join(', ') : `Todos (${totalCount})`;

  return (
    <div ref={ref} className="relative">
      <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wide mb-1.5">{label}</label>
      <button type="button" onClick={() => setOpen(o => !o)} className={`w-full flex items-center justify-between gap-2 px-3 py-2 rounded-lg border text-sm transition ${hasSelection ? 'border-blue-400 bg-blue-50 text-blue-800 font-medium' : 'border-gray-200 bg-white text-gray-600 hover:border-gray-300'}`}>
        <span className="truncate text-left">{displayLabel}</span>
        <div className="flex items-center gap-1 flex-shrink-0">
          {hasSelection && <span onClick={e => { e.stopPropagation(); onClear(); }} className="text-blue-400 hover:text-red-500 cursor-pointer" title="Limpar seleção"><X className="w-3.5 h-3.5" /></span>}
          <ChevronDown className={`w-4 h-4 text-gray-400 transition-transform ${open ? 'rotate-180' : ''}`} />
        </div>
      </button>
      {open && (
        <div className="absolute z-50 mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-lg overflow-hidden">
          <button type="button" onClick={() => { onClear(); setOpen(false); }} className={`w-full flex items-center justify-between px-3 py-2 text-sm hover:bg-gray-50 transition ${!hasSelection ? 'bg-blue-50 text-blue-800 font-medium' : 'text-gray-700'}`}>
            <span>Todos</span><span className="text-xs text-gray-400">{totalCount}</span>
          </button>
          ...
          <div className="max-h-52 overflow-y-auto">
            {options.map(opt => {
              ...
              return (
                <button key={opt.value} type="button" onClick={() => onToggle(opt.value)} className={`w-full flex items-center gap-2 px-3 py-2 text-sm hover:bg-gray-50 transition ${checked ? 'bg-blue-50 text-blue-800' : 'text-gray-700'}`}>
                  <div className={`w-4 h-4 rounded border flex-shrink-0 flex items-center justify-center ${checked ? 'bg-blue-600 border-blue-600' : 'border-gray-300'}`}>
...
// ─── Row builder ──────────────────────────────────────────────────────────────
interface Row {
  _id: string; _type: string; _teams: string[]; _status: string;
  _teamStatus: Record<string, 'completed' | 'pending'>;
  _movement: Movement; _createdAt: Date; _approvedAt: Date | null;
  _isCanceled: boolean; _canceledAt: Date | null; _movementDate: Date | null;
  Nome: string; Coligada: string; Tipo: string; 'Criado por': string;
  'Data de criação': string; 'Data da Mudança/Desligamento': string;
  Status: string; 'Faltam parecer': string; 'Com pareceres emitidos': string;
  'Último Parecer': string; 'Cancelado em': string; 'Cancelado por': string;
}

function buildRows(movements: Movement[], currentUser: CurrentUser): Row[] {
  return movements
    .filter(m => currentUser.role === 'admin' || m.created_by === currentUser.name || m.selected_teams.some(t => currentUser.team_ids.includes(t)))
    .map(m => {
      ...
      const isCanceled  = m.cancelamento !== null && m.cancelamento !== undefined;
      const statusLabel = isCanceled ? 'Cancelado' : (pendentes.length === 0 ? 'Aprovado' : 'Pendente');
      ...
      if (!isCanceled && pendentes.length === 0) {
        const timestamps = m.selected_teams.map(id => {
          const resp = m.responses[id]; const hist = resp?.history || [];
          if (hist.length > 0) return new Date(hist[hist.length - 1].timestamp);
          if (resp?.date) return new Date(resp.date);
          return null;
        }).filter((d): d is Date => d !== null && !isNaN(d.getTime()));
        if (timestamps.length > 0) approvedAt = new Date(Math.max(...timestamps.map(d => d.getTime())));
      }

      const allTimestamps = m.selected_teams.map(id => {
        const resp = m.responses[id]; if (resp?.status !== 'completed') return null;
        const hist = resp?.history || [];
        if (hist.length > 0) return new Date(hist[hist.length - 1].timestamp);
        if (resp?.date) return new Date(resp.date); return null;
      }).filter((d): d is Date => d !== null && !isNaN(d.getTime()));
      const lastResponseDate = allTimestamps.length > 0 ? formatDate(new Date(Math.max(...allTimestamps.map(d => d.getTime()))).toISOString()) : '—';

      let canceledAt: Date | null = null; let canceledAtStr = '—'; let canceledByStr = '—';
      if (isCanceled) {
      ...
      const coligada        = m.details?.company || '—';

      return {
        _id: m.id, _type: m.type, _teams: m.selected_teams,
        _status: statusLabel, _teamStatus: teamStatus, _movement: m,
        _createdAt: new Date(m.created_at), _approvedAt: approvedAt,
        _isCanceled: isCanceled, _canceledAt: canceledAt, _movementDate: movementDate,
...
function matchesTeamStatus(row: Row, filterTeams: Set<string>, filterStatuses: Set<string>): boolean {
  if (filterTeams.size === 0) { if (filterStatuses.size > 0 && !filterStatuses.has(row._status)) return false; return true; }
...
// ─── Excel helpers ────────────────────────────────────────────────────────────
const GENERAL_COLS = [
  'Nome', 'Coligada', 'Tipo', 'Criado por', 'Data de criação',
...
function buildGeneralSheet(rows: Row[]): XLSX.WorkSheet {
  ...
  ws['!cols'] = [
    { wch: 35 }, { wch: 38 }, { wch: 18 }, { wch: 28 }, { wch: 16 },
    { wch: 28 }, { wch: 12 }, { wch: 16 }, { wch: 45 }, { wch: 45 },
    { wch: 16 }, { wch: 22 },
  ];
  return ws;
}

function buildTeamSheet(teamId: string, allRows: Row[]): XLSX.WorkSheet | null {
  const rows = allRows.filter(r => r._teams.includes(teamId));
  ...
  const ws = XLSX.utils.json_to_sheet(data, { header: TEAM_COLS });
  ws['!cols'] = [
    { wch: 35 }, { wch: 38 }, { wch: 18 }, { wch: 28 }, { wch: 16 }, { wch: 28 }, { wch: 12 },
  ];
  return ws;
}

// ─── Component ────────────────────────────────────────────────────────────────
interface RelatorioViewProps { currentUser: CurrentUser; movements: Movement[]; loading: boolean; }
...
export default function RelatorioView({ currentUser, movements, loading }: RelatorioViewProps) {
  ...
  const [filterColigadas,  setFilterColigadas]  = useState<Set<string>>(new Set());
  const [showCanceled,     setShowCanceled]     = useState(false);
  const [dateCreatedStart, setDateCreatedStart] = useState('');
  const [dateCreatedEnd,   setDateCreatedEnd]   = useState('');
  const [dateApprStart,    setDateApprStart]    = useState('');
  const [dateApprEnd,      setDateApprEnd]      = useState('');
  const [dateMovStart,     setDateMovStart]     = useState('');
  const [dateMovEnd,       setDateMovEnd]       = useState('');
  ...
  const matchesDateFilters = (r: Row) => {
    const cs = toStart(dateCreatedStart); const ce = toEnd(dateCreatedEnd);
    if (cs && r._createdAt < cs) return false; if (ce && r._createdAt > ce) return false;
    const as_ = toStart(dateApprStart);  const ae  = toEnd(dateApprEnd);
    if (as_ || ae) { if (!r._approvedAt) return false; if (as_ && r._approvedAt < as_) return false; if (ae && r._approvedAt > ae) return false; }
    const ms = toStart(dateMovStart);    const me  = toEnd(dateMovEnd);
    if (ms || me) { if (!r._movementDate) return false; if (ms && r._movementDate < ms) return false; if (me && r._movementDate > me) return false; }
    return true;
  };
  ...
  const COL_KEY: Record<string, keyof Row> = {
    ...
    'Criação': '_createdAt', 'Mudança/Deslig.': '_movementDate', 'Status': 'Status',
    'Último Parecer': 'Último Parecer', 'Faltam': 'Faltam parecer',
    'Responderam': 'Com pareceres emitidos', 'Cancelado em': 'Cancelado em', 'Cancelado por': 'Cancelado por',
  };
  ...
  const toggleSelectAll = () => { if (selected.size === sortedFiltered.length) setSelected(new Set()); else setSelected(new Set(sortedFiltered.map(r => r._id))); };

  const handlePrintSelected = () => { const rows = sortedFiltered.filter(r => selected.has(r._id)); if (rows.length === 0) { alert('Selecione ao menos uma movimentação.'); return; } rows.forEach(r => printMovementPDF(r._movement)); };
  const handlePrintOne = (row: Row) => printMovementPDF(row._movement);

  const handleExportExcel = () => {
    setExporting(true);
    try {
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, buildGeneralSheet(sortedFiltered), 'Geral');
      for (const team of TEAMS_LIST) {
        const ws = buildTeamSheet(team.id, sortedFiltered);
        if (!ws) continue;
    ...
    } catch (e) { alert('Erro ao gerar o arquivo Excel.'); console.error(e); }
  ...
  // Opções de coligada presentes nas linhas base (sem filtro de coligada)
  const coligadaOptions = useMemo(() => {
    const counts: Record<string, number> = {};
    rowsForColigadaCount.forEach(r => { if (r.Coligada && r.Coligada !== '—') counts[r.Coligada] = (counts[r.Coligada] || 0) + 1; });
    return Object.entries(counts).sort((a, b) => a[0].localeCompare(b[0])).map(([v, count]) => ({ value: v, label: v, count }));
  }, [rowsForColigadaCount]);
    ...
    <div className="bg-white rounded-lg shadow" style={{ minWidth: 0 }}>
      ...
      <div className="flex flex-wrap justify-between items-center gap-3 p-4 border-b border-gray-100">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button onClick={handleExportExcel} disabled={exporting || sortedFiltered.length === 0} className="flex items-center gap-1.5 px-3 py-1.5 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-xs font-medium" title="Exporta aba Geral + uma aba por equipe com pendências">
            {exporting ? <><Loader2 className="w-3.5 h-3.5 animate-spin" />Exportando...</> : <><FileSpreadsheet className="w-3.5 h-3.5" />Pendências por Setor</>}
          </button>
        </div>
      </div>
      ...
      <div className="p-4">
        ...
        <div className="grid grid-cols-3 gap-3 mb-3">
          <MultiSelect label="Status" selected={filterStatuses} onToggle={v => toggleSet(setFilterStatuses, v)} onClear={() => setFilterStatuses(new Set())}
            options={(['Pendente', 'Aprovado', 'Cancelado'] as const).map(opt => {
              let count = 0;
              if (opt === 'Cancelado') count = rowsForStatusCount.filter(r => r._isCanceled).length;
              else count = rowsForStatusCount.filter(r => {
                if (filterTeams.size === 0) return r._status === opt;
                return [...filterTeams].some(t => { if (!r._teams.includes(t)) return false; const done = r._teamStatus[t] === 'completed'; return opt === 'Aprovado' ? done : !done; });
              }).length;
              return { value: opt, label: `${opt === 'Cancelado' ? '✕' : opt === 'Pendente' ? '⏳' : '✓'} ${opt}`, count };
            })}
            totalCount={rowsForStatusCount.length}
          />
          <MultiSelect label="Tipo" selected={filterTypes} onToggle={v => toggleSet(setFilterTypes, v)} onClear={() => setFilterTypes(new Set())}
            options={MOVEMENT_TYPES.map(t => ({ value: t.id, label: t.label, count: rowsForTypeCount.filter(r => r._type === t.id).length }))}
            totalCount={rowsForTypeCount.length}
          />
          <MultiSelect label="Equipe" selected={filterTeams} onToggle={v => toggleSet(setFilterTeams, v)} onClear={() => setFilterTeams(new Set())}
        ...
        {loading ? (
          ...
          <div className="text-center py-10 bg-gray-50 rounded-lg"><p className="text-gray-500 text-sm">Nenhuma movimentação encontrada.</p></div>
        ) : (
          <div className="overflow-x-auto rounded-lg border border-gray-100">
            ...
            <div className="flex items-center justify-between px-3 py-2 bg-gray-50 border-t border-gray-100">
              ...
              {someSelected && <p className="text-xs text-blue-600 font-medium">{selected.size} selecionada{selected.size !== 1 ? 's' : ''}</p>}
            </div>
          </div>
        )}
      </div>
