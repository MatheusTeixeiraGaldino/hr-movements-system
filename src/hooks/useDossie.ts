  ...
  const atualizarTipoDesligamento = useCallback(
    async (
      ...
      try {
        ...
        const novoChecklist = novosDocs.map((doc: TipoDocumento) => {
          const existente = dossie.checklist.find(
            (c: ItemChecklist) => c.documento === doc
          );
        ...
        setDossies((prev: AcompanhamentoDossie[]) =>
          prev.map((d: AcompanhamentoDossie) =>
            d.id === id
              ? {
                  ...d,
                  tipo_desligamento: novoTipo,
  ...
  const atualizarDataDemissao = useCallback(
    async (id: string, dataDemissao: string, user: string, email: string) => {
      ...
      try {
        ...
        setDossies((prev: AcompanhamentoDossie[]) =>
          prev.map((d: AcompanhamentoDossie) => d.id === id ? { ...d, data_demissao: dataDemissao, historico_auditoria: historico } : d)
        );
      } catch (err: any) {
        setError(err.message);
        console.error(err);
  ...
  const atualizarObservacao = useCallback(
    async (id: string, observacao: string, user: string, email: string) => {
      ...
      try {
        ...
        setDossies((prev: AcompanhamentoDossie[]) =>
          prev.map((d: AcompanhamentoDossie) => d.id === id ? { ...d, observacao, historico_auditoria: historico } : d)
        );
      } catch (err: any) {
        setError(err.message);
        console.error(err);
  ...
  const atualizarPastaDesligado = useCallback(
    async (id: string, pasta: string, user: string, email: string) => {
      ...
      try {
        ...
        setDossies((prev: AcompanhamentoDossie[]) =>
          prev.map((d: AcompanhamentoDossie) => d.id === id ? { ...d, pasta_desligado: pasta, historico_auditoria: historico } : d)
        );
      } catch (err: any) {
        setError(err.message);
        console.error(err);
