    ...
    <div className="bg-white rounded-lg shadow p-6">
      ...
      <div className="space-y-3 mb-6">
        {selectedMovement.selected_teams.map((id: string) => {
          ...
          return (
            <div key={id} className={`border rounded-lg p-4 ${isMine ? 'border-blue-500 bg-blue-import React, { useState, useEffect } from 'react';
...
function DashboardView({ currentUser, movements, loading, loadMovements, setSelectedMovement, setView }: any) {
  ...
  const getProgress = (m: Movement) => {
    const completed = m.selected_teams.filter((t: string) => m.responses[t]?.status === 'completed').length;
  ...
  const myMovs: Movement[] = movements.filter((m: Movement) => {
    if (isAdmin) {
  ...
  const pending: Movement[] = myMovs.filter((m: Movement) => {
  ...
  const completed: Movement[] = myMovs.filter((m: Movement) => {
    if (m.created_by === currentUser?.name && !m.selected_teams.includes(currentUser?.team_id || '')) {
      return m.status === 'completed';
    }
  ...
  const getFilteredMovements = (): Movement[] => {
    let filtered: Movement[] = showCompleted ? completed : pending;
  ...
  const getCountByType = (type: MovementType, includeCompleted: boolean = false): number => {
    const movs: Movement[] = includeCompleted ? myMovs : pending;
      ...
      {pending.length > 0 && (
        <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 mb-6 rounded">
          ...
          <span className="font-medium text-yellow-800">
            {isAdmin 
              ? `Você tem ${pending.filter((m: Movement) => m.selected_teams.includes(currentUser?.team_id || '')).length} movimentação(ões) pendente(s) de parecer`
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            ...
            >
              <div className="text-center">
                <UserX className="w-6 h-6 mx-auto mb-1 text-
